...
/**
 * A comparator follows the same contract as Array.prototype.sort:
 *   returns negative  → a comes before b  (a is "smaller")
 *   returns 0         → a and b are equal
 *   returns positive  → b comes before a  (b is "smaller")
 *
 * Examples:
 *   numbers  : (a, b) => a - b          ← ascending (min-heap default)
 *   strings  : (a, b) => a.localeCompare(b)
 *   objects  : (a, b) => a.priority - b.priority
 */
...
class MinHeap<T> {
   ...
   */
  constructor(comparator: Comparator<T> = (a, b) => (a as unknown as number) - (b as unknown as number)) {
   ...
   */
  private leftChildIdx(i: number): number {
    return 2 * i + 1;
  }
   ...
   */
  private rightChildIdx(i: number): number {
    return 2 * i + 2;
  }
   ...
   */
  private isSmaller(idxA: number, idxB: number): boolean {
  ...
  private bubbleUp(i: number): void {
    while (i > 0) {
      const parent: number = this.parentIdx(i);
  ...
  public extractMin(): T | null {
    ...
    const last = this.heap.pop() as T; // non-null because we checked isEmpty
  ...
  private bubbleDown(i: number): void {
    ...
    while (true) {
      const left:  number = this.leftChildIdx(i);
      const right: number = this.rightChildIdx(i);
  ...
  public heapify(array: T[]): void {
    ...
    const lastNonLeaf: number = Math.floor(this.heap.length / 2) - 1;
  ...
  public print(): void {
    ...
    let level: number    = 0;
    let levelSize: number = 1; // nodes per level: 1, 2, 4, 8 …
    let i: number        = 0;
    const lines: string[] = [];
...
function heapSort<T>(arr: T[], comparator?: Comparator<T>): T[] {
  ...
  h.heapify(arr);                                 // O(n)
  return arr.map(() => h.extractMin() as T);      // O(n log n)
}
...
const heapFromArray = new MinHeap<number>();
...
const unsorted: number[] = [5, 3, 8, 1, 2, 9, 4, 7, 6];
console.log("  Input:  ", unsorted);
console.log("  Sorted: ", heapSort(unsorted));

const words: string[] = ["mango", "apple", "kiwi", "banana"];
console.log("\n  Input:  ", words);
