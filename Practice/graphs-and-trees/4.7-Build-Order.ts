 ...
 *   
 * time complexity - O(n + e) for visiting nodes and edges
 * space complexity - O(2n) for visited, stack
 *   
 */

class Solution {
  public buildOrder(projects: string[], dependencies: string[][]): string[] {
  ...
  private buildGraph(projects: string[], dependencies: string[][], adjList: Map<string, string[]>){
  ...
  private buildOrderWithGraphDFS(adjList: Map<string, string[]>){
    // 0->unvisited, 1->in-process, 2->finished
    let visited: Map<string, number> = new Map(); 
    let stack: string[] = [];
    let result: string[] = [];
    let cycleDetected: {value: boolean} = {value: false};
    ...
    if(cycleDetected.value == true) return [];
  ...
  private DFS(node: string, visited: Map<string, number>, adjList:Map<string, string[]>,  stack: string[], cycleDetected: {value: boolean}  ){
    // base case
    ...
    if(visited.get(node) == 0){
      ...
      // cycle detected
      console.error('Error: Cycle detected');
...
console.log("Example 1 - Expected: [f,e,a,b,d,c], Got:", 
  solution1.buildOrder(
    ["a","b","c","d","e","f"],
    [["a","d"],["f","b"],["b","d"],["f","a"],["d","c"]]
  )
);
...
console.log("Example 2 - Expected: Error, Got:",
  solution2.buildOrder(
    ["a","b"],
    [["a","b"],["b","a"]]
  )
);